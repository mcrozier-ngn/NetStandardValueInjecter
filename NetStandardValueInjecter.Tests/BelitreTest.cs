...
namespace Xciles.NetStandardValueInjecter.Tests
{
    public class BelitreTest
    {
        ...
        public void Cast()
        {
            ...
            Assert.Equal("o", (target as List<M2>).First().Name);
        }
        ...
        public void Doit()
        {
            ...
            d.InjectFrom(c);
            Assert.Equal(c.CustomerId, d.CustomerId);
            Assert.Equal(c.FirstName, d.FirstName);
            Assert.Equal(c.SecondName, d.SecondName);

            d.InjectFrom(new My(countries), c);

            Assert.Equal(c.CustomerCountries.Count(), d.Countries.Count());
            Assert.Equal(countries.First().Name, d.Countries.First().Name);
        }
